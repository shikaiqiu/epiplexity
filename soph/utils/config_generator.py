...
class FixedSeededNumpyRNG:
    """Context manager to save/restore numpy PRNG state and resume from a fixed seed or saved state"""
    ...
    def __enter__(self):
        ...
        if self.state is None:  # First time entering
            np.random.seed(self.seed)
        else:  # Resuming from previous state
...
from soph.utils.executor import LocalGpuExecutor
import concurrent
from tqdm.auto import tqdm

def dispatch_multigpu(fn, cfg_spec, num_trials=-1, max_workers=None, ordered=True):
    """
    Dispatches a function fn with configurations generated from cfg_spec over available gpus

    if num_trials is a negative integer, then num_trials is interpreted as -num_trials
            full passes through the grid. If positive, then it is the number of trials to run.
    
    if not ordered, the runs will be dispatched in a random order
    """
    with LocalGpuExecutor(max_workers=max_workers) as executor:
        cfgs = grid_iter(cfg_spec, num_trials, shuffle=not ordered)
        futures = {executor.submit(fn, cfg) for cfg in cfgs}
        for j, future in enumerate(tqdm(concurrent.futures.as_completed(futures),
                                            total=len(futures),desc="runs")):
            print(f"Finished {j+1}/{len(futures)}")
